...
//		System.out.println(result);
	...
	private void generatePath(String start, String cur, ArrayList<ArrayList<String>> result, Map<String, ArrayList<String>> map, ArrayList<String> path) {
		if (path.size() != 0 && path.get(0).equals(start)) {
			result.add((ArrayList<String>) path.clone());
			return;
		}
		 
		for (String word : map.get(cur)) {
			path.add(0, word);
			generatePath(start, word, result, map, path);
			path.remove(0);
		}
	}
